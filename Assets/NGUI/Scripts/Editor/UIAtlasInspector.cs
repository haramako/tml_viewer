...
public class UIAtlasInspector : Editor
{
	...
	// 2016/03/10 ykurokawa パラメーター後から変更できるように
	#region NGUI_CHANGE_FOR_N3DS
	private string createdWidth = null;
	private string createdHeight = null;
	...
	public override void OnInspectorGUI ()
	{
		...
		// 2016/02/10 ykurokawa (どの解像度向けに、アトラスを作ったかを表示します。）
		#region NGUI_CHANGE_FOR_N3DS
		GUILayout.Label( "texture size::" + mAtlas.GetTextureWidth() + " x " + mAtlas.GetTextureHeight() );

		if (string.IsNullOrEmpty(createdWidth))
		{
			createdWidth = mAtlas.GetTextureWidth().ToString();
		}
		if (string.IsNullOrEmpty(createdHeight))
		{
			createdHeight = mAtlas.GetTextureHeight().ToString();
		}
		EditorGUILayout.BeginHorizontal();
		this.createdWidth = GUILayout.TextField(this.createdWidth, GUILayout.Width(40));
		GUILayout.Label("x", GUILayout.Width(10));
		this.createdHeight = GUILayout.TextField(this.createdHeight, GUILayout.Width(40));
		EditorGUILayout.EndHorizontal();
		try
		{
			if (GUILayout.Button("Apply TexureSize",GUILayout.Width(150)))
			{
				mAtlas.SetTextureSize(int.Parse(this.createdWidth), int.Parse(this.createdHeight));
