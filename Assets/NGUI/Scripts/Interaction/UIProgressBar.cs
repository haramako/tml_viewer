...
#if FAKE_RIGIDBODY
using BoxCollider = FakeRigid.BoxCollider;
#endif
	...
	public float alpha
	{
		...
		set
		{
			...
			if (mFG != null)
			{
				mFG.alpha = value;
#if FAKE_RIGIDBODY
				if (mFG.GetComponent<BoxCollider>() != null) mFG.GetComponent<BoxCollider>().enabled = mFG.alpha > 0.001f;
#else
			...
			if (mBG != null)
			{
				mBG.alpha = value;
#if FAKE_RIGIDBODY
				if (mBG.GetComponent<BoxCollider>() != null) mBG.GetComponent<BoxCollider>().enabled = mBG.alpha > 0.001f;
#else
			...
			if (thumb != null)
			{
				...
				if (w != null)
				{
					w.alpha = value;
#if FAKE_RIGIDBODY
					if (w.GetComponent<BoxCollider>() != null) w.GetComponent<BoxCollider>().enabled = w.alpha > 0.001f;
#else
