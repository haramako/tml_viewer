...
public class UILabel : UIWidget
{
	...
	public static Font GetDefaultFont(){
		if (DefaultFont == null) {
			#if UNITY_EDITOR
			var assets = UnityEditor.AssetDatabase.FindAssets("FOT-KurokaneStd-EB");
			var path = UnityEditor.AssetDatabase.GUIDToAssetPath(assets[0]);
			DefaultFont = (Font)UnityEditor.AssetDatabase.LoadAssetAtPath(path, typeof(Font));
			#else
			DefaultFont = (Font)Resources.Load("FOT-KurokaneStd-EB");
			#endif
		}
		return DefaultFont;
	}
	...
	public Font trueTypeFont
	{
		get
		{
			...
			// MEMO: フォントを指定いなくてもいい対応のため
			if (mFont != null) return mFont.dynamicFont;
	...
	bool isValid { get { return mFont != null || trueTypeFont != null; } } // MEMO: フォントを指定ないくても描画される対応
