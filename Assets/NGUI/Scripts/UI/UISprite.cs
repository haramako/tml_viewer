...
public class UISprite : UIBasicSprite
{
	...
	// 2016/02/10 ykurokawa (3DSだけ解像度の違うテクスチャ設定にするのですが、そのためだけにアトラスを作るのはバカらしいので・・・）
	#region NGUI_CHANGE_FOR_N3DS
	#if UNITY_N3DS
	/// <summary>
	/// Atlasの幅取得
	/// </summary>
	/// <returns>Atlasの幅を返します</returns>
	protected override int GetAtlasWidth()
	{
		if (this.atlas != null)
		{
			return this.atlas.GetTextureWidth();
		}
		return 0;
	}
	/// <summary>
	/// Atlasの高さを取得
	/// </summary>
	/// <returns>Atlasの高さを返します</returns>
	protected override int GetAtlasHeight()
	{
		if (this.atlas != null)
		{
			return this.atlas.GetTextureHeight();
		}
		return 0;
	}
	#endif
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		// 少し強引ですが、テクスチャの見切れる対策
		float scale = (float)h / (float)tex.height;
		float rScale = (float)tex.height / (float)h;
		float px = (outer.xMin - ((int)(outer.xMin * rScale) * scale) + 0.5f) * scale;
		float py = (outer.yMin - ((int)(outer.yMin * rScale) * scale) + 0.5f) * scale;
		outer.yMin += py ;
		inner.yMin += py;
		outer.xMin += px;
		inner.xMin += px;

		outer = NGUIMath.ConvertToTexCoords(outer, w, h);
		inner = NGUIMath.ConvertToTexCoords(inner, w, h);
#else
