...
#else
			#region NGUI_CHANGE_FOR_N3DS
			#if UNITY_N3DS
			if (!font.dynamic) {
				font.GetCharacterInfo(')', out mTempChar);
				mTempChar.minY = (mTempChar.minY * finalSize) / font.fontSize;
				mTempChar.maxY = (mTempChar.maxY * finalSize) / font.fontSize;
			}
			else if ((!font.GetCharacterInfo(')', out mTempChar, finalSize, fontStyle) || mTempChar.maxY == 0f))
			#else // 元の実装
			if (!font.GetCharacterInfo(')', out mTempChar, finalSize, fontStyle) || mTempChar.maxY == 0f)
			#endif
			#endregion NGUI_CHANGE_FOR_N3DS
			{
				font.RequestCharactersInTexture("A", finalSize, fontStyle);
				{
					if (!font.GetCharacterInfo('A', out mTempChar, finalSize, fontStyle))
	...
	static public float GetGlyphWidth (int ch, int prev)
	{
		...
		else if (dynamicFont != null)
		{
			...
			if (dynamicFont.dynamic)
			{
				...
				if (dynamicFont.GetCharacterInfo((char)ch, out mTempChar) ){
					return ((float)(mTempChar.advance * finalSize) / (float)dynamicFont.fontSize) * fontScale * pixelDensity;
				}
			}
	...
	static public GlyphInfo GetGlyph (int ch, int prev)
	{
		...
		else if (dynamicFont != null)
		{
			...
			if (!dynamicFont.dynamic)
			{
				if (dynamicFont.GetCharacterInfo((char)ch, out mTempChar))
				{
					int dynafontSize = dynamicFont.fontSize;
					mTempChar.minX = (mTempChar.minX * finalSize) / dynafontSize;
					mTempChar.maxX = (mTempChar.maxX * finalSize) / dynafontSize;
					mTempChar.minY = (mTempChar.minY * finalSize) / dynafontSize;
					mTempChar.maxY = (mTempChar.maxY * finalSize) / dynafontSize;
					mTempChar.advance = (mTempChar.advance * finalSize) / dynafontSize;
					// mTempChar.bearing = (mTempChar.bearing * finalSize) / dynafontSize;
 				}
			}else{
				if (!dynamicFont.GetCharacterInfo((char)ch, out mTempChar, finalSize, fontStyle))
				{
