...
public class UIWidget : UIRect
{
	...
	protected override void OnStart ()
	{
		...
		{
			//Debug.LogError("[NGUI] Widgets and panels should not be on the same object! Widget must be a child of the panel.", this); // MEMO: 警告を抑制
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			Debug.LogError("[NGUI] You should not place more than one widget on the same object. Weird stuff will happen!", this);
		}
#endif
		CreatePanel();
	}
